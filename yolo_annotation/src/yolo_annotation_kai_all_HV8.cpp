...
void Annotation_yolo::Annotation_exec()
{
    ...
    if (save_count_ < the_number_of_data) {
        ...
        for (int i = 0; i < target_frames_and_states_.size(); i++) {
            file << model_name_  << " " << target_frames_and_states_[i].points_2d[0].x << " " << target_frames_and_states_[i].points_2d[0].y << " " << target_frames_and_states_[i].points_2d[1].x << " " << target_frames_and_states_[i].points_2d[1].y << std::endl;
        }
        file.close();
        save_count_++;
    }
...
void Annotation_yolo::rotation_convert(geometry_msgs::TransformStamped source_tf, std::vector<target_frame_and_state> &all_msgs)
{
    ...
    for (int i = 0; i < all_msgs.size(); i++) {
        ...
        // source_quat = convert_quat(q_z, source_quat, -M_PI / 2) * source_quat;
